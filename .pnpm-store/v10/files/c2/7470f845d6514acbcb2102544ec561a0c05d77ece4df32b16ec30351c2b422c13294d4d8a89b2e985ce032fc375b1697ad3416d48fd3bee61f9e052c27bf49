...
async function getVueDeclarations(vfs, opts) {
  ...
  const pkgInfo = await pkgTypes.readPackageJSON("vue-tsc").catch(() => {
  });
  if (!pkgInfo) {
    console.warn(
  ...
  const majorVersion = semver.major(pkgInfo.version);
  switch (majorVersion) {
    case 1: {
...
async function emitVueTscV2(vfs, compilerOptions, srcFiles) {
  ...
  const requireFromVueTsc = node_module.createRequire(await mlly.resolve("vue-tsc"));
  ...
  const createProgram = volarTs.proxyCreateProgram(
    ...
    (ts2, options) => {
      const vueLanguagePlugin = vueLanguageCore.createVueLanguagePlugin(
        ...
        () => "",
        () => options.rootNames.map((rootName) => pathe.normalize(rootName)),
        options.options,
...
async function mkdist(options = {}) {
  ...
  if (options.cleanDist !== false) {
    await fse__default.unlink(options.distDir).catch(() => {
    });
    await fse__default.emptyDir(options.distDir);
    await fse__default.mkdirp(options.distDir);
  }
  ...
  const files = filePaths.map((path) => {
    ...
    return {
      ...
      getContents: () => fse__default.readFile(sourcePath, { encoding: "utf8" })
    };
  });
  ...
  await Promise.all(
    outputs.filter((o) => !o.skip).map(async (output) => {
      ...
      await fse__default.mkdirp(pathe.dirname(outFile));
      await (output.raw ? copyFileWithStream(output.srcPath, outFile) : fse__default.writeFile(outFile, output.contents, "utf8"));
