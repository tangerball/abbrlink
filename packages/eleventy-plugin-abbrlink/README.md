# eleventy-plugin-abbrlink

> An Eleventy plugin to generate unique abbreviated links for Markdown files
...
```bash
pnpm add eleventy-plugin-abbrlink
# or
npm install eleventy-plugin-abbrlink
# or
yarn add eleventy-plugin-abbrlink
```

## 🚀 Usage

Add it to your plugins in `eleventy.config.js`:
...
1. The plugin scans the configured Markdown files during Eleventy's build process
...
5. It processes files automatically during Eleventy's build process
